...
### 6.1 M0 종료
- [ ] 한글 IME: 조합 중 글자 깨짐 없음 / 조합 중 백스페이스 / 표 셀 안 / 코드블록 안 / 링크 텍스트 안 / 한자 변환 / 영문↔한글 전환 직후 타이핑
- [ ] `changed` 이벤트가 조합 중 문자마다 발생하지 않음(또는 디바운스로 흡수)
- [ ] Round-trip 코퍼스 diff=0 비율 ≥ 95%
- [ ] 100KB 문서 로드 ≤ 1초, 타이핑 지연 체감 없음(≤ 50ms)
- [ ] 콜드 스타트 실측 기록, WebView2 프로세스 포함 메모리 기록
- [ ] ARCH 부록 A 7개 항목 확인 완료
- [ ] Go/No-Go 결정 기록
